...
<div class="min-h-screen bg-[#f7f7fc] text-slate-900">
 ...
 <aside :class="['fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform duration-200',mobileOpen?'translate-x-0':'-translate-x-full lg:translate-x-0']">
  ...
  <div class="border-t border-slate-100 p-4"><RouterLink to="/settings" @click="closeMobile" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50"><Settings :size="18"/>Settings</RouterLink><button @click="profileOpen=!profileOpen" class="mt-3 flex w-full items-center gap-3 rounded-xl bg-slate-50 p-3 text-left hover:bg-orange-50"><div class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700">{{initials}}</div><div class="min-w-0 flex-1"><p class="truncate text-xs font-bold">{{displayName}}</p><p class="truncate text-[10px] text-slate-400">{{displayRole}}</p></div><ChevronDown :size="15" class="text-slate-400"/></button></div>
 </aside>
 <div class="lg:pl-64">
  <header class="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/85 px-4 backdrop-blur-xl sm:px-6">
   ...
   <div class="ml-auto flex items-center gap-2">
    ...
    <div class="hidden h-8 w-px bg-slate-200 sm:block"></div><div class="relative"><button @click="profileOpen=!profileOpen;notificationsOpen=false" class="flex items-center gap-2 rounded-xl p-1.5 text-left transition hover:bg-orange-50"><div class="grid h-8 w-8 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700">{{initials}}</div><div class="hidden sm:block"><p class="text-xs font-bold">{{displayName}}</p><p class="text-[10px] text-slate-400">{{displayRole}}</p></div><ChevronDown :size="15" class="text-slate-400"/></button><div v-if="profileOpen" class="absolute right-0 top-12 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><div class="border-b border-slate-100 bg-slate-50 p-4"><div class="flex items-center gap-3"><div class="grid h-11 w-11 place-items-center rounded-full bg-orange-100 font-black text-orange-700">{{initials}}</div><div class="min-w-0"><p class="truncate text-sm font-bold">{{displayName}}</p><p class="truncate text-xs text-slate-400">{{displayEmail}}</p></div></div><span class="mt-3 inline-flex rounded-full bg-orange-100 px-2.5 py-1 text-[10px] font-bold text-orange-700">{{displayRole}}</span></div><div class="p-2"><RouterLink to="/settings" @click="profileOpen=false" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50"><UserCircle :size="17"/>Profile & Settings</RouterLink><button @click="logout" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50"><LogOut :size="17"/>Log out</button></div></div></div>
   </div>
  </header>
