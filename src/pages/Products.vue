...
const allSelected=computed(()=>paginated.value.length>0&&paginated.value.every(p=>selectedIds.value.includes(p.id)))
const toggleAll=()=>{if(allSelected.value) selectedIds.value=selectedIds.value.filter(id=>!paginated.value.some(p=>p.id===id)); else selectedIds.value=[...new Set([...selectedIds.value,...paginated.value.map(p=>p.id)])]}
const toggleSelected=(id:string)=>selectedIds.value=selectedIds.value.includes(id)?selectedIds.value.filter(x=>x!==id):[...selectedIds.value,id]
const bulkArchive=()=>{selectedIds.value.forEach(id=>store.archiveProduct(id));selectedIds.value=[]}

const openNew=()=>{editingId.value=undefined;showForm.value=true}
const openDetails=(id:string)=>router.push('/products/'+id)
const openEdit=(id:string)=>{editingId.value=id;showForm.value=true}
...
<section class="p-5 sm:p-6 lg:p-8">
 ...
 <div v-if="selectedIds.length" class="mt-4 flex items-center justify-between rounded-xl border border-orange-200 bg-orange-50 px-4 py-3"><span class="text-sm font-semibold text-orange-800">{{selectedIds.length}} products selected</span><button @click="bulkArchive" class="rounded-lg bg-slate-950 px-3 py-2 text-xs font-bold text-white">Archive selected</button></div><div class="mt-4 flex items-center justify-between text-sm text-slate-500"><span>{{filtered.length.toLocaleString('en-IN')}} products</span><span>Page {{page}} of {{totalPages}}</span></div>
 <div class="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white"><div class="overflow-x-auto"><table class="w-full min-w-[980px] text-left text-sm"><thead class="bg-orange-50/70 text-xs uppercase text-slate-500"><tr><th class="w-12 px-5 py-3"><input type="checkbox" :checked="allSelected" @change="toggleAll" class="h-4 w-4 accent-orange-500"/></th><th class="px-5 py-3">Product</th><th class="px-5 py-3">SKU</th><th class="px-5 py-3">Category</th><th class="px-5 py-3 cursor-pointer" @click="sort('price')">Price <ArrowUpDown :size="13" class="inline"/></th><th class="px-5 py-3 cursor-pointer" @click="sort('stock')">Stock <ArrowUpDown :size="13" class="inline"/></th><th class="px-5 py-3 cursor-pointer" @click="sort('sales')">Sales <ArrowUpDown :size="13" class="inline"/></th><th class="px-5 py-3 cursor-pointer" @click="sort('rating')">Rating <ArrowUpDown :size="13" class="inline"/></th><th class="px-5 py-3">Status</th></tr></thead><tbody><tr v-for="p in paginated" :key="p.id" @click="openDetails(p.id)" class="cursor-pointer border-t transition hover:bg-orange-50/50"><td class="px-5 py-4" @click.stop><input type="checkbox" :checked="selectedIds.includes(p.id)" @change="toggleSelected(p.id)" class="h-4 w-4 accent-orange-500"/></td><td class="px-5 py-4"><div class="flex items-center gap-3"><div class="grid h-9 w-9 place-items-center rounded-lg bg-orange-50"><PackageOpen :size="17" class="text-orange-500"/></div><div><p class="font-semibold text-slate-800 hover:text-orange-700">{{p.name}}</p><p class="text-xs text-slate-400">{{p.id}}</p></div></div></td><td class="px-5 py-4 text-slate-500">{{p.sku}}</td><td class="px-5 py-4">{{categoryName(p.categoryId)}}</td><td class="px-5 py-4 font-medium">{{money(p.price)}}</td><td class="px-5 py-4"><span :class="p.stock<=p.reorderLevel?'font-semibold text-amber-600':''">{{p.stock}}<span v-if="p.stock<=p.reorderLevel" class="ml-1 text-xs">Low</span></span></td><td class="px-5 py-4">{{p.sales.toLocaleString('en-IN')}}</td><td class="px-5 py-4">★ {{p.rating}}</td><td class="px-5 py-4"><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium">{{p.status}}</span></td></tr><tr v-if="!paginated.length"><td colspan="9" class="py-16 text-center text-slate-500">No products match your filters.</td></tr></tbody></table></div><div class="flex items-center justify-between border-t px-5 py-4"><span class="text-xs text-slate-400">Showing {{filtered.length?((page-1)*perPage+1):0}}–{{Math.min(page*perPage,filtered.length)}} of {{filtered.length}}</span><div class="flex items-center gap-1"><button @click="changePage(page-1)" :disabled="page===1" class="grid h-8 w-8 place-items-center rounded-md hover:bg-orange-50 disabled:opacity-30"><ChevronLeft :size="16"/></button><button v-for="n in totalPages" :key="n" @click="changePage(n)" :class="['h-8 min-w-8 rounded-md px-2 text-sm',page===n?'bg-orange-500 text-white':'hover:bg-orange-50']">{{n}}</button><button @click="changePage(page+1)" :disabled="page===totalPages" class="grid h-8 w-8 place-items-center rounded-md hover:bg-orange-50 disabled:opacity-30"><ChevronRight :size="16"/></button></div></div></div>
</section>
<ProductForm :is-open="showForm" :product-id="editingId" @close="showForm=false" />
</template>