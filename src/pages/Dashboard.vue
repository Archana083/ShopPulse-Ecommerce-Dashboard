...
<div class="relative mb-7 overflow-hidden rounded-[28px] bg-gradient-to-br from-violet-700 via-indigo-600 to-fuchsia-600 p-6 text-white shadow-xl shadow-violet-500/20 sm:p-8">
<div class="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-white/10 blur-2xl"></div><div class="absolute -bottom-20 right-24 h-48 w-48 rounded-full bg-fuchsia-300/20 blur-3xl"></div>
<div class="relative flex flex-wrap items-end justify-between gap-5"><div><div class="mb-3 inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold backdrop-blur"><Sparkles :size="14"/> Live store overview</div><h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">Good afternoon, Admin 👋</h1><p class="mt-2 max-w-xl text-sm text-violet-100">A quick pulse on revenue, customers, orders and inventory health.</p></div><select v-model="period" class="rounded-xl border border-white/20 bg-white/15 px-4 py-2.5 text-sm font-semibold text-white outline-none backdrop-blur"><option class="text-slate-900">Today</option><option class="text-slate-900">Last 7 days</option><option class="text-slate-900">Last 30 days</option><option class="text-slate-900">This year</option></select></div>
</div>
<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
<div v-for="stat in stats" :key="stat.label" class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-violet-500/10">
<div class="absolute right-0 top-0 h-20 w-20 rounded-full bg-violet-50 blur-xl transition group-hover:scale-150"></div><div class="relative flex items-center justify-between"><div :class="['grid h-11 w-11 place-items-center rounded-xl',stat.tone==='amber'?'bg-amber-50 text-amber-600':stat.tone==='blue'?'bg-blue-50 text-blue-600':stat.tone==='fuchsia'?'bg-fuchsia-50 text-fuchsia-600':'bg-violet-50 text-violet-600']"><component :is="stat.icon" :size="20"/></div><ArrowUpRight :size="17" class="text-slate-300 transition group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-violet-500"/></div>
...
<div v-for="(height,i) in revenueBars" :key="i" class="group flex h-full flex-1 items-end"><div :style="{height:height+'%'}" class="relative w-full rounded-t-xl bg-gradient-to-t from-violet-600 to-fuchsia-400 transition-all duration-500 group-hover:from-violet-500 group-hover:to-fuchsia-300"><span class="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 rounded-lg bg-slate-950 px-2 py-1 text-[10px] font-bold text-white opacity-0 transition group-hover:opacity-100">₹{{Math.round(height*1320).toLocaleString('en-IN')}}</span></div></div>
...
<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-3"><div class="flex items-center justify-between"><div><h2 class="font-bold">Recent orders</h2><p class="mt-1 text-sm text-slate-500">Latest customer transactions</p></div><RouterLink to="/orders" class="rounded-lg px-3 py-2 text-sm font-semibold text-violet-600 transition hover:bg-violet-50">View all</RouterLink></div><div class="mt-5 overflow-x-auto"><table class="w-full min-w-[650px] text-left text-sm"><thead class="border-b border-slate-100 text-[11px] uppercase tracking-wider text-slate-400"><tr><th class="pb-3">Order</th><th class="pb-3">Customer</th><th class="pb-3">Amount</th><th class="pb-3">Status</th></tr></thead><tbody><tr v-for="r in recentOrders" :key="r.id" class="border-b border-slate-50 transition hover:bg-violet-50/30"><td class="py-4 font-bold text-slate-800">{{r.id}}</td><td class="py-4">{{customerName(r.customerId)}}</td><td class="py-4 font-bold">{{money(r.total)}}</td><td class="py-4"><span class="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-700">{{r.status}}</span></td></tr></tbody></table></div></div>
<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2"><h2 class="font-bold">Top products</h2><p class="mt-1 text-sm text-slate-500">Best sellers by units</p><div class="mt-5 space-y-4"><div v-for="(p,i) in topProducts" :key="p.id" class="flex items-center gap-3"><span class="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-violet-100 to-fuchsia-100 text-xs font-extrabold text-violet-700">{{i+1}}</span><div class="min-w-0 flex-1"><div class="flex justify-between gap-3"><p class="truncate text-sm font-semibold">{{p.name}}</p><span class="text-xs font-bold text-slate-500">{{p.sales.toLocaleString('en-IN')}}</span></div><div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500 transition-all duration-700" :style="{width:Math.min(100,Math.max(8,p.sales/Math.max(1,topProducts[0]?.sales||1)*100))+'%'}"></div></div></div></div></div></div>
</div>
</div></section>
